...
def create_access_token(user_id: int, password_version: int = 0) -> str:
    ...
    payload = {"sub": str(user_id), "pwdv": password_version, "exp": expire}
    return jwt.encode(payload, settings.secret_key, algorithm=settings.jwt_algorithm)


def decode_access_token_claims(token: str) -> tuple[int, int] | None:
    """解析 JWT，返回用户 id 和密码版本。旧令牌未带版本时按 0 兼容。"""
    try:
        payload = jwt.decode(token, settings.secret_key, algorithms=[settings.jwt_algorithm])
        sub = payload.get("sub")
        if sub is None:
            return None
        return int(sub), int(payload.get("pwdv", 0))
    except (jwt.InvalidTokenError, TypeError, ValueError):
        return None


def decode_access_token(token: str) -> int | None:
    """解析 JWT 并返回用户 id，保留给不需要密码版本的调用方。"""
