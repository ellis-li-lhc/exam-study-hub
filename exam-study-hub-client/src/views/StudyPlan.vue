<template>
  <div class="plan-page page-stack">
    <section class="page-intro"><div><span class="section-kicker">STEP 05</span><h2>{{ store.profile.mode==='plan'?'我的最短达标计划':'我的自主学习路线' }}</h2><p>预计 {{ store.estimatedWeeks }} 周 · 每周 {{ store.weeklyHours }} 小时 · 目标 {{ store.targetScore }} 分</p></div><el-tag size="large">{{ store.profile.mode==='plan'?'计划模式':'自主模式' }}</el-tag></section>

    <section class="stage-overview"><div v-for="stage in store.stages" :key="stage.id" class="stage-node" :class="stage.status"><span><el-icon v-if="stage.status==='completed'"><Check /></el-icon><template v-else>{{ stage.id }}</template></span><div><small>阶段 {{ stage.id }}</small><strong>{{ stage.name }}</strong><p>{{ stage.weeks }} 周建议</p></div></div></section>

    <section class="plan-layout">
      <div class="stage-detail">
        <el-card v-for="stage in store.stages" :key="stage.id" shadow="never" class="stage-card" :class="stage.status">
          ...
          <div v-if="stage.status==='active'" class="active-actions"><el-button type="primary" plain @click="openStageTest">进行阶段测试</el-button><span>系统出题并自动判分；偏低时只提供复习建议，不会强制退回。</span></div>
        </el-card>
      </div>

      <aside class="daily-panel">
        <div class="daily-head"><div><span class="section-kicker">{{ store.profile.mode==='plan'?'今天':'当前建议' }}</span><h3>{{ store.profile.mode==='plan'?'完成 3 项任务':'基础建立阶段' }}</h3></div><el-progress type="circle" :percentage="store.overallProgress" :width="64" :stroke-width="7" /></div>
        <template v-if="store.profile.mode==='plan'"><label v-for="task in store.tasks" :key="task.id" class="task-row" :class="{done:task.done}"><el-checkbox :model-value="task.done" @change="store.toggleTask(task.id)"/><span><b>{{ task.subject }}</b><strong>{{ task.title }}</strong><small>{{ task.type }} · {{ task.duration }} 分钟</small></span></label></template>
        <div v-else class="self-list"><p>自主模式不生成每日排期，建议按下面顺序推进：</p><ol><li>政治高频基础概念</li><li>英语词汇与基础阅读</li><li>{{ store.selectedMajor?.subjects[2] }}核心基础</li></ol></div>
      </aside>
    </section>
...
function submitTest() {
  ...
  const wrongKnowledge = stageQuestions.value
    .filter(question => testAnswers[question.id] !== question.answer)
  ...
  const result = store.submitStageTest({ score, accuracy, correctCount, totalQuestions, weakKnowledge })
