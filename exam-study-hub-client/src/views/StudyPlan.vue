  ...
  <div class="plan-page page-stack">
    ...
    <section class="plan-layout">
      ...
      <aside class="daily-panel">
        ...
        <template v-if="store.profile.mode==='plan'"><label v-for="task in store.tasks" :key="task.id" class="task-row" :class="{done:task.done, review: task.reviewKey}"><el-checkbox :model-value="task.done" @change="store.toggleTask(task.id)"/><span><b>{{ task.subject }}</b><strong>{{ task.title }}</strong><small>{{ task.type }} · {{ task.duration }} 分钟</small></span></label></template>
