<template>
  <div class="schools-page page-stack">
    ...
    <el-alert title="院校与投档线为 2025 年江苏省教育考试院数据，按科类投档；下列为该科类招生院校，具体专业以院校当年招生简章为准。" type="info" show-icon :closable="false" />
      ...
      <div class="school-list">
        <article v-for="school in store.filteredInstitutions" :key="school.code" class="school-card" :class="{ selected: store.selectedInstitutionCode === school.code }" @click="store.selectInstitution(school.code)">
          <div class="school-head">
            ...
            <div class="school-name"><div><h3>{{ school.name }}</h3><el-tag v-if="store.selectedInstitutionCode === school.code" type="success" size="small">已选择</el-tag></div><p>{{ provinceName(school.province) }} · {{ school.city }} · {{ store.selectedMajor?.category }}</p></div>
        ...
        <dl><div><dt>教学点</dt><dd>{{ selected.teachingSite }}</dd></div><div><dt>学位条件</dt><dd>{{ selected.degree }}</dd></div>
          <div><dt>数据来源</dt><dd>
            <template v-if="selected.source && selected.source.confidence === 'verified'">
              {{ selected.source.provider }} · {{ selected.source.year }} 年{{ selected.source.line_type }}
